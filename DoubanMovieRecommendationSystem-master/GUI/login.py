...
class Login(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle('Movie recommendation based on Douban')  # 设置窗口标题
        self.setWindowIcon(QIcon('../Data/douban.jpg'))  # 设置窗口图标
        self.user_label = QLabel('User name: ', self)  # 文本设置
        self.pwd_label = QLabel('Password: ', self)
        self.user_line = QLineEdit(self)  # 单行文本编辑器
        self.pwd_line = QLineEdit(self)
        self.login_button = QPushButton('Login', self)  # 命令按钮
    ...
    def lineedit_init(self):
        self.user_line.setPlaceholderText('Input user name')  # 在输入框显示浅灰色的提示文本
        self.pwd_line.setPlaceholderText('Input password')
    ...
    def check_login_func(self):
        if self.USER_PWD.get(self.user_line.text()) == self.pwd_line.text():
            QMessageBox.information(self, 'Notification', 'Login successfully!')
            self.close()
            self.MainWindow = MainWindow(self.user_line.text())
            self.MainWindow.show()
        else:
            QMessageBox.critical(self, 'Notification', 'Wrong user name or password!')
        self.pwd_line.clear()
