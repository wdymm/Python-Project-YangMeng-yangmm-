...
class MovieDetailed(QWidget):
    def __init__(self, name):
        ...
        self.name = name
        # 电影信息
        self.movies_detailed = pd.read_csv('../Data/douban_movies.csv',encoding='utf-8').fillna('')
        ...
        try:
            ...
            self.directors_label = QLabel("<h2>" + "Director: " + self.directors + "</h2>")
            self.writer_label = QLabel("<h2>" + "Screen writer: " + self.writer + "</h2>")
            self.actors_label = QLabel("<h2>" + "Actor: " + self.actors.split(' ')[0] + "</h2>")
            self.style_label = QLabel("<h2>" + "Genre: " + self.style + "</h2>")

            self.country_label = QLabel("<h2>" + "Nation: " + self.country + "</h2>")
            self.language_label = QLabel("<h2>" + "Language: " + self.language + "</h2>")
            self.date_label = QLabel("<h2>" + "Date: " + str(int(self.date)) + "</h2>")
            self.duration_label = QLabel("<h2>" + "Duration: " + str(int(self.duration)) + "</h2>")

            self.rate_label = QLabel("<h1>" + "Rating: " + str(self.rate) + "</h1>")
            self.introduction_label = QLabel("<h1><b>Movie intro:</b></h1>", self)
            ...
            self.resize(200, 200)
            self.no_find = QLabel("<h1>Sorry, no movie founded!</h1>", self)
            self.h_layout = QHBoxLayout()
...
if __name__ == "__main__":
    ...
    detailed = MovieDetailed("Revengers")
    detailed.show()
    sys.exit(app.exec_())
