...
class SearchWindow(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Search movie")
        ...
        self.search_label = QLabel("<h3>Input movie name:</h3>", self)
    ...
    def Fuzzy_search(self, keyword):
        ...
        if keyword == '':
            flag = 0
            self.search_browser.clear()
            self.search_browser.setText("<h2>Input cannot be none!</h2>")
        else:
            self.search_browser.clear()
            if list(self.movies_df[self.movies_df['name'].str.contains('{}'.format(keyword))].name) == []:
                self.search_browser.append("<h2>Sorry, no related movie!</h2>")
        ...
        try:
            for i in range(15):
                ...
                self.content = "<h3>" + "《" + self.name + "》" + "  Rating:" + self.rating + "</h3>" + '\n' + "<h3>" + "  Link:" + self.url + "</h3>"
