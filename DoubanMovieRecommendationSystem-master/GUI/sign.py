...
class SigninPage(QDialog):
    def __init__(self):
        ...
        self.signin_user_label = QLabel('User name:', self)  # 文本设置
        ...
        self.signin_button = QPushButton('Sign in', self)  # 命令按钮
    ...
    def check_signin_func(self):
        if self.signin_pwd_line.text() != self.signin_pwd2_line.text():
            QMessageBox.critical(self, 'Notification', 'Passwords are not the same in these 2 times!')
        elif self.signin_user_line.text() not in self.USER_PWD:
            QMessageBox.information(self, 'Notification', 'Sign up successfully!')
            f = open('../Data/users_info_en.csv', 'a+', encoding='utf-8')
            ...
            QMessageBox.critical(self, 'Notification', 'User name has already be token!')
